package dependencies

object Versions {

    val gradle = "3.5.3"
    val compilesdk = 28
    val minsdk = 21
    val targetsdk = 28
    val kotlin = "1.3.61"
    val nav_components = "2.3.0-alpha06"
    val material_dialogs = "3.2.1"
    val room = "2.2.5"
    val appcompat = "1.1.0-rc01"
    val constraintlayout = "1.1.3"
    val material_design = "1.2.0"
    val play_core = "1.7.1"
    val espresso_core = "3.1.1"
    val espresso_idling_resource = "3.2.0"
    val mockk_version = "1.9.2"
    val test_runner = "1.2.0"
    val test_core = "1.2.0"
    val lifecycle_version = "2.2.0-alpha03"
    val retrofit2_version = "2.6.0"
    val markdown_processor = "0.1.3"
    val junit_jupiter_version = "5.6.0"
    val junit_4_version = "4.12"
    val fragment_version = "1.2.0"
    val androidx_test_ext = "1.1.1"
    val fabric_version = "1.31.2"
    val reactive_streams="2.2.0"
    val androidx_core = "1.1.0"
    val room_rxjava = "2.8.1"
    val retrofir_rxjava = "2.8.1"
    val lifecycle_extentions = "2.0.0"
    val rxrelay = "3.0.0"
    val picasso = "2.71828"
    val youtubeplayer = "10.0.5"
    val rxandroid = "2.1.0"
    val rxjava = "2.2.2"
    val constraint_layout = "1.1.3"
    val android_material = "1.2.0"
    }

